...
{
	std::auto_ptr<AAX_VPageTable> newTable;
	...
	if ( mComponentFactory )
	{
		...
		const AAX_Result result = const_cast<IACFComponentFactory&>(*mComponentFactory).CreateComponent(AAXCompID_PageTable, 0, IID_IAAXPageTableV2, (void **)&mIPageTable);
		if (ACF_OK != result)
		{
			mIPageTable = NULL;
		}
