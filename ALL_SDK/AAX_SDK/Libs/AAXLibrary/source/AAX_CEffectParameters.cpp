...
AAX_Result	AAX_CEffectParameters::Initialize(IACFUnknown* inController)
{
	...
	int32_t numControls;
	GetNumberOfParameters(&numControls);
	if (numControls > 0)
	{
...
{
	if (AAX_eNotificationEvent_ASPreviewState == inNotificationType)
    {
...
AAX_Result AAX_CEffectParameters::SetChunk ( AAX_CTypeID chunkID, const AAX_SPlugInChunk * chunk )
{
	...
	int32_t numControls, controlIndex;
	this->GetNumberOfParameters(&numControls);
