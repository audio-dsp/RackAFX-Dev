...
void DemoGain_Parameters::SetGains ( SDemoGain_CoefsGain * ioGains, bool inType, double inValue, DemoGain_ESide inSide )
{
	if ( inType == eType_Pan )
	{
		ioGains->mGainL[inSide] = (float) 1.0 - inValue;
		ioGains->mGainR[inSide] = (float) inValue;
	}
	else
	{
		if ( inSide == eSide_Left )
		{
			ioGains->mGainL[inSide] = (float) DBToGain ( inValue );
			ioGains->mGainR[inSide] = 0.0;
		}
		else
		{
			ioGains->mGainL[inSide] = 0.0;
			ioGains->mGainR[inSide] = (float) DBToGain ( inValue );
		}
	}
