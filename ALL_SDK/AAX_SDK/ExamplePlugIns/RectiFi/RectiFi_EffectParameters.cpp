/*================================================================================================*/
/*
 *	Copyright 2010-2015, 2017 by Avid Technology, Inc.
 *	All rights reserved.
 *
...
AAX_Result RectiFi_EffectParameters::Gain_ProcessFunction( AAX_CPacket& ioPacket )
{
	...
	if (parameter)
	{
		...
		{		
			...
			*messagePayloadP = pow (10.0, (gainValue / 20.0));//used to be float (tick count check is needed)
...
AAX_Result RectiFi_EffectParameters::Mix_ProcessFunction( AAX_CPacket& ioPacket )
{
	...
	if (parameter)
	{
		...
		{		
			...
			{
				messagePayloadP->mDryMix = 1.0 - 0.8*effectMix;	// [1.0, 0.6] 
				messagePayloadP->mRectiMix = 1.2*effectMix;		// [0.0, 0.6]
			}
			else
			{
				messagePayloadP->mDryMix   = 1.2 - 1.2*effectMix;	// [0.6, 0.0]
				messagePayloadP->mRectiMix = 0.2 + 0.8*effectMix;	// [0.6, 1.0] 
			}
