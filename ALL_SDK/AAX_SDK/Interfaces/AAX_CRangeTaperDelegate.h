...
public: 
	...
	virtual T		GetMinimumValue()	const AAX_OVERRIDE			{ return mMinValue; }
	virtual T		GetMaximumValue()	const  AAX_OVERRIDE			{ return mMaxValue; }
...
AAX_CRangeTaperDelegate<T, RealPrecision>::AAX_CRangeTaperDelegate(T* ranges, double* rangesSteps, long numRanges, bool useSmartRounding) :
...
{
	mRangesStepsCount.reserve(numRanges);
	mRangesPercents.reserve(numRanges);
	int i = 0;
	for (; i < mNumRanges; i++)
	{
...
T	AAX_CRangeTaperDelegate<T, RealPrecision>::Round(double iValue) const
{
	return ((0 >= RealPrecision) ?	static_cast<T>(iValue) :
			(0 <= iValue) ?			floor( iValue*RealPrecision + 0.5f ) / RealPrecision :
...
double	AAX_CRangeTaperDelegate<T, RealPrecision>::RealToNormalized(T realValue)	const
{
	...
	if (i == mNumRanges)
		...
		percentTotal += (realValue - mRanges.at(i))/(mRanges.at(i + 1) - mRanges.at(i)) * mRangesPercents.at(i);
...
T	AAX_CRangeTaperDelegate<T, RealPrecision>::SmartRound(double value) const
{
	...
	return static_cast<double>(longVal) * mRangesSteps.at(i);
}


#endif AAX_CRANGETAPERDELEGATE_H
