...
enum AAX_EStemFormat
{
	...
	//
	//
	AAX_eStemFormat_Ambi_1_ACN = AAX_STEM_FORMAT ( 14, 4 ),   ///< Reserved for Ambisonics: first-order with ACN channel order and SN3D (AmbiX) normalization
	AAX_eStemFormat_Ambi_2_ACN = AAX_STEM_FORMAT ( 18, 9 ),   ///< Reserved for Ambisonics: second-order with ACN channel order and SN3D (AmbiX) normalization
	AAX_eStemFormat_Ambi_3_ACN = AAX_STEM_FORMAT ( 19, 16 ),  ///< Reserved for Ambisonics: third-order with ACN channel order and SN3D (AmbiX) normalization
	
	// Reserved stem formats (do not use)
	AAX_eStemFormat_Reserved_1 = AAX_STEM_FORMAT ( 15, 4 ),   ///< Reserved - do not use
	AAX_eStemFormat_Reserved_2 = AAX_STEM_FORMAT ( 16, 9 ),   ///< Reserved - do not use
	AAX_eStemFormat_Reserved_3 = AAX_STEM_FORMAT ( 17, 16 ),  ///< Reserved - do not use
	
	AAX_eStemFormatNum			= 21, // One greater than the highest available AAX_STEM_FORMAT_INDEX value. This needs to increase as stem types are added.
...
/*!	@brief Events IDs for %AAX notifications
	...
	%AAX host and may be received in
	\ref AAX_IEffectParameters::NotificationReceived() and/or
	\ref AAX_IEffectGUI::NotificationReceived(). Note that only one of these
	two components may be registered to receive a notification.
	- Notifications listed with <em>Sent by: Plug-in</em> are dispatched by the %AAX
	plug-in to notify the host that some event has occured.
...
enum AAX_EConstraintLocationMask
{
	...
	/** \brief This \c ProcessProc may not be "moved" between different latency domains; it must always receive
		audio buffers of the same size over its lifetime.
	 
		\li Different instances of the \c ProcessProc may receive buffers at different fixed sizes, but the buffers
		received by any particular \c ProcessProc will not change.
		\li This constraint only applies to Native algorithms. DSP processing always occurs at a fixed size.
		
		\compatibility This constraint is not currently supported by any %AAX host
	 */
