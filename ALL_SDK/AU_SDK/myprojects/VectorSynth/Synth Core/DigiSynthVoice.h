...
public:
	...
	inline bool isOscSingleCycleSample(UINT uOsc)
	{
		...
		{
			case 0:
				pOsc = (CSampleOscillator*)m_pOsc1;
				break;
			case 1:
				pOsc = (CSampleOscillator*)m_pOsc2;
				break;
			case 2:
				pOsc = (CSampleOscillator*)m_pOsc3;
				break;
			case 3:
				pOsc = (CSampleOscillator*)m_pOsc4;
				break;
			default:
				break;
		}
	...
	inline bool initOscWithFilePath(UINT uOsc, char* pPath, bool bSingleCycleSample, bool bPitchlessSample)
	{
		...
		if(pOsc)
		{
			...
			((CSampleOscillator*)pOsc)->m_bPitchlessSample = bSingleCycleSample;
	...
	inline virtual bool doVoice(double& dLeftOutput, double& dRightOutput)
	{
		...
		if(m_pGlobalVoiceParams->uVoiceMode == Osc1)
			dLeftAccum = m_OscA.doOscillate(&dRightAccum);
		else if(m_pGlobalVoiceParams->uVoiceMode == Osc2)
			dLeftAccum = m_OscB.doOscillate(&dRightAccum);
		else if(m_pGlobalVoiceParams->uVoiceMode == split)
		{
			bool Osc1On = inOscSplitRange(0);
			bool Osc2On = inOscSplitRange(1);
			
			dLeft1 = m_OscA.doOscillate(&dRight1);
			dLeft2 = m_OscB.doOscillate(&dRight2);
