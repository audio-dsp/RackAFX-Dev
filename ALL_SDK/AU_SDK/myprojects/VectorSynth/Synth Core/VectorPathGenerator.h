...
public:
	...
	*/
	...
	inline void startProgram()
	{
		...
		m_dA_Mix = m_dStartA_Mix;
		m_dB_Mix = m_dStartB_Mix;
		m_dC_Mix = m_dStartC_Mix;
		m_dD_Mix = m_dStartD_Mix;
		m_dAC_Mix = m_dStartAC_Mix;
		m_dBD_Mix = m_dStartBD_Mix;
		...
		// --- special situation with only one step in SUSTAIN mode
		if(m_nNumSteps == 1 && this->m_uJSMode == JS_SUSTAIN)
			m_bRunning = false;
		else
			m_bRunning = true;
	}
	...
	inline void setVPGPathMode(UINT u)
	{
		...
		{
			m_uJSMode = u;
			m_bForwardPath = true; // reset when path changes
		}
	}
