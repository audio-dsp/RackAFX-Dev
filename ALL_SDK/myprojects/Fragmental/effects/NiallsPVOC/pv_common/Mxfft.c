 ...
 *  and mxfft.f from the CARL distribution.
 *
...
void
fft_(float *a,float *b,int nseg,int n,int nspn,int isn)
 ...
 */
void
fftmx(float	*a,float *b,int ntot,int n,int nspan,int isn,int m,int *kt,
	  float *at,float *ck,float	*bt,
	  float	*sk,int	*np,int nfac[])
...
{	
	int	i,inc,
		...
		maxf,mm,
		nn,nt;
	float  aa, aj, ajm, ajp, ak, akm, akp,
...
void
reals_(float *a,float * b,int n,int isn)
